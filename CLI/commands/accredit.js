...
async function changeAccredited() {
  ...
  if (tokenDeployedAddress != "0x0000000000000000000000000000000000000000") {
    ...
    let stoAddresses = await securityToken.methods.getModulesByName("USDTieredSTO").call();
    if (stoAddresses.length > 0) {
      let result = await securityToken.methods.getModule(stoAddresses[0]).call();
